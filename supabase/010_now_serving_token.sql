-- ============================================================
-- get_queue_status() returned "the next 5 waiting tokens" but never
-- the token actually being seen right now (the one patient with
-- status = 'in_consult') — the queue page mislabeled the next-5 list
-- as "now serving," which wasn't accurate. This adds a real
-- nowServingToken field, distinct from nearbyTokens (the next 5 still
-- waiting), so the public page can show both correctly.
-- ============================================================
