-- ============================================================
-- Qlinic — migration 042: scoped "queue details" counts for the
-- public patient queue link
--
-- Why: the redesigned queue.html's "View queue details" popup needs
-- to say, precisely, how many patients are booked between now and
-- THIS patient's own turn — not the whole day, and not a random slice.
-- "Ahead of my turn" already means "not yet resolved" (booked/waiting/
-- in_consult only — done/no_show are excluded simply by not matching
-- those statuses), so no extra now()-based lower bound is needed: the
-- upper bound is the calling patient's own effective moment (the same
-- booked_time-or-arrived_at logic the existing position/ordering
-- already uses), and everything still active with an earlier or equal
-- moment is, by definition, ahead of or at their own turn.
--
-- Purely additive to the jsonb this function returns — every existing
-- key stays exactly as-is.
--
-- Run this once in the Supabase SQL Editor, after 041_queue_status_richer_info.sql.
-- ============================================================
